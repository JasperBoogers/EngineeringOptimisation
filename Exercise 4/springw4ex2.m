...
xlabel('Coil diameter D (m)'), ylabel('Wire diameter d (m)'), ...
    title('Exercise 4.2     Spring stiffness and frequency optimization problem (w = 1.0)')
hold on
...
while it <= it_max
    ...
    alpha_lb = (LB-xq)./sq;
    alpha_ub = (UB-xq)./sq;
    lb = max(min(alpha_lb), min(alpha_ub));
    ub = min(max(alpha_lb), max(alpha_ub));
    
    if lb > ub
        ub = max(max(alpha_lb), max(alpha_ub));
    end

    %Line search (note the lower and upper bound of alfhaq):
    [alphaq,fval,exitflag] = ...
        fminbnd('springobjw4',0,ub,options,xq,sq,ktarget,frtarget,w);
    ...
    if abs(fval-f_prev) <= eps, return; end
    if it > it_max
